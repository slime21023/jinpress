        ...
        if lang:
            try:
                ...
                formatter = HtmlFormatter(
                    cssclass="highlight", linenos=True, hl_lines=highlight_lines
                )
...
class Renderer:
    ...
    def extract_front_matter(self, content: str) -> tuple[dict[str, Any], str]:
        ...
        if content.startswith("---\n"):
            try:
                ...
                raise RendererError(f"Invalid YAML front matter: {e}") from e
    ...
    def render_markdown(self, file_path: Path) -> tuple[str, dict[str, Any]]:
        ...
        try:
            ...
            raise RendererError(f"Error reading file {file_path}: {e}") from e
    ...
    def get_page_data(self, file_info: dict[str, Any], config: Any) -> dict[str, Any]:
        ...
        """
        # Support both legacy Config class and new JinPressConfig
        if hasattr(config, "get"):
            # Legacy Config class
            site_title = config.get("site.title", "JinPress Site")
            site_description = config.get("site.description", "")
            site_lang = config.get("site.lang", "zh-TW")
            site_base = config.get("site.base", "/")
            theme_config = config.get("theme", {})
        else:
            # New JinPressConfig dataclass
            site_title = config.site.title
            site_description = config.site.description
            site_lang = config.site.lang
            site_base = config.site.base
            theme_config = (
                config.theme.to_dict() if hasattr(config.theme, "to_dict") else {}
            )
